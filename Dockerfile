...
RUN mkdir /freebsd && \
	curl -Ls http://ftp-archive.freebsd.org/pub/FreeBSD-Archive/old-releases/amd64/9.0-RELEASE/base.txz | \
...
RUN mkdir -p /freebsd/usr/local/etc && \
	echo 'ABI = "FreeBSD:9:amd64"; REPOS_DIR = ["/freebsd/etc/pkg"]; REPO_AUTOUPDATE = NO; RUN_SCRIPTS = NO;' > /freebsd/usr/local/etc/pkg.conf

ADD fix-links /freebsd/fix-links

RUN mkdir -p /usr/local/cross-compiler/x86_64-pc-freebsd9/ && \
    cp -a /freebsd/usr/include /usr/local/cross-compiler/x86_64-pc-freebsd9/ && \
    cp -a /freebsd/lib /usr/local/cross-compiler/x86_64-pc-freebsd9/ && \
    cp -a /freebsd/usr/lib/* /usr/local/cross-compiler/x86_64-pc-freebsd9/lib/ && \
...
RUN cd /src/binutils-2.25.1 && \
    ...
    ./configure --prefix=/usr/local/cross-compiler --with-gnu-ld  --enable-static \
      --enable-shared --with-gmp=/usr/local/cross-compiler --host=x86_64-pc-freebsd9 && \
    make -j4 && \
    make install && \
    cd /src/mpc-1.0.3/ && \
    ./configure --prefix=/usr/local/cross-compiler/ --with-gnu-ld \
      --enable-static --enable-shared --with-gmp=/usr/local/cross-compiler \
      --with-mpfr=/usr/local/cross-compiler --host=x86_64-pc-freebsd9  &&\
    make -j4 && \
    make install && \
    ...
    ../configure --without-headers --with-gnu-as --with-gnu-ld --disable-nls \
        ...
        --disable-libitm --disable-libquadmath --disable-libcilkrts --target=x86_64-pc-freebsd9 \
